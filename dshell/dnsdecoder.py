...
class DNSDecoder(dshell.TCPDecoder):
    ...
    def packetHandler(self, udp, data):
        '''for each UDP packet , examine each segment (UDP packet) seperately as each will be a DNS Q/A pair Q/A by ID and return as pairs'''
        addr = udp.addr
        if addr[0][1] < addr[1][1]:
            addr = addr[1], addr[0]  # swap ports if source port is lower, to keep tuple (client,server)
        ...
        if dns.qr == dpkt.dns.DNS_Q:
            ...
            if "DNSHandler" in dir(self):
                self.DNSHandler(
                    conn=udp,
                    request=rq[1], response=dns,
                    requesttime=rq[0], responsetime=udp.ts,
                    responsecount=rq[2]
                )

    def blobHandler(self, conn, blob):
        '''for each blob, examine each segment (UDP packet) seperately as each will be a DNS Q/A pair Q/A by ID and return as pairs'''
            ...
            if dns.qr == dpkt.dns.DNS_Q:
                ...
                if "DNSHandler" in dir(self):
                    self.DNSHandler(
                        conn=conn,
                        request=rq[1], response=dns,
                        requesttime=rq[0], responsetime=blob.starttime,
                        responsecount=rq[2]
                    )
